"""Render the CS Analytical opportunity brief — long form.

Renders the same content tree (from report_content.BLOCKS) as both a
DOCX (Caslon-specified, editable in Word) and a PDF (typeset, vector).
"""
...
OUT_DOCX = ROOT / "04_Long_Form_Report" / "CS_Analytical_Opportunity_Brief_long.docx"
OUT_PDF = ROOT / "04_Long_Form_Report" / "CS_Analytical_Opportunity_Brief_long.pdf"
...
def render_docx(blocks, out: Path):
    ...
    def add_h1(text):
        add_para(text, size=24, color=NAVY, space_before=24, space_after=10,
                 line_spacing=1.1)
    ...
    def add_h2(text):
        add_para(text, size=15.5, color=NAVY, space_before=14, space_after=4,
                 line_spacing=1.15)
    ...
    def add_callout(title, body):
        add_para(title, size=12, color=BURGUNDY, italic=True,
                 space_before=10, space_after=2, left_indent=0.3)
        add_para(body, size=11, color=CHARCOAL,
    ...
    def add_cover(payload):
        ...
        add_para(payload["subtitle"], size=14, color=CHARCOAL, italic=True,
...
def render_pdf(blocks, out: Path):
    ...
    def draw_body_chrome(c, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, PAGE_H - MARGIN + 0.35 * inch,
                          "Prepared for Alan Weiss, CFO  ·  Confidential")
        c.line(MARGIN, MARGIN + 0.15 * inch,
               PAGE_W - MARGIN, MARGIN + 0.15 * inch)
        c.setFont(body_font, 9)
        c.drawString(MARGIN, MARGIN - 0.05 * inch,
                     "Bruce Longley  ·  TSI-Citadel")
    ...
    doc = BaseDocTemplate(
        ...
        title="CS Analytical Opportunity Brief — for Alan Weiss",
        author="Bruce Longley, TSI-Citadel",
        subject="Market, organic growth, and agentic-AI opportunity",
    )
    ...
    base = ParagraphStyle("base", fontName=body_font, fontSize=11,
                          leading=15.5, textColor=CHARCOAL,
                          alignment=TA_LEFT, spaceAfter=8)
    h1 = ParagraphStyle("h1", parent=base, fontName=body_font, fontSize=20,
                        leading=24, textColor=NAVY, spaceBefore=22,
                        spaceAfter=8)
    h2 = ParagraphStyle("h2", parent=base, fontName=body_font, fontSize=14,
                        leading=18, textColor=NAVY, spaceBefore=14,
                        spaceAfter=4)
    h3 = ParagraphStyle("h3", parent=base, fontName=italic_font,
                        fontSize=11.5, leading=15, textColor=BURGUNDY,
                        spaceBefore=10, spaceAfter=3)
    lead = ParagraphStyle("lead", parent=base, fontName=italic_font,
                          fontSize=12.5, leading=18, textColor=NAVY,
                          spaceAfter=12)
    quote = ParagraphStyle("quote", parent=base, fontName=italic_font,
                           fontSize=12, leading=17, textColor=NAVY,
                           leftIndent=24, rightIndent=12, spaceBefore=6,
                           spaceAfter=10)
    body = ParagraphStyle("body", parent=base, fontName=body_font,
                          fontSize=11, leading=15.5, textColor=CHARCOAL,
                          spaceAfter=9)
    caption = ParagraphStyle("caption", parent=base, fontName=italic_font,
    ...
    cover_eyebrow = ParagraphStyle("cover_eyebrow", parent=base,
                                   fontName=italic_font, fontSize=11,
                                   leading=14, textColor=SLATE,
                                   alignment=TA_LEFT, spaceBefore=200,
                                   spaceAfter=12)
    cover_title = ParagraphStyle("cover_title", parent=base,
                                 fontName=body_font, fontSize=30,
                                 leading=36, textColor=NAVY,
                                 spaceAfter=12)
    cover_subtitle = ParagraphStyle("cover_subtitle", parent=base,
                                    fontName=italic_font, fontSize=14,
                                    leading=20, textColor=CHARCOAL,
                                    spaceAfter=120)
    cover_author = ParagraphStyle("cover_author", parent=base,
                                  fontName=body_font, fontSize=12,
                                  leading=16, textColor=CHARCOAL,
                                  spaceAfter=2)
    cover_date = ParagraphStyle("cover_date", parent=base,
                                fontName=italic_font, fontSize=10,
                                leading=13, textColor=SLATE)
    callout_title = ParagraphStyle("callout_title", parent=base,
                                   fontName=italic_font, fontSize=11.5,
                                   leading=15, textColor=BURGUNDY,
                                   spaceAfter=4)
    callout_body = ParagraphStyle("callout_body", parent=base,
    ...
    def emit_cover(payload):
        ...
        story.append(Paragraph(payload["subtitle"].replace("\n", "<br/>"),
                               cover_subtitle))
    ...
    for kind, payload in blocks:
        ...
        if kind == "h1":
            ...
            story.append(ListFlowable(items, bulletType="bullet",
                                      bulletFontSize=8, leftIndent=18,
            ...
            img = Image(str(path), width=img_w_in * inch,
                        height=img_h_in * inch)
            img.hAlign = "CENTER"
